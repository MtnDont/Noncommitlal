...
[yapprox, err] = TaylorMultN(f, dfdt, fexact, t0, tn, y0, n);
log_h = [];
for i=1:length(n)
...
function [y, err] = TaylorMultN(f, dfdt, fexact, t0, tn, y0, nList)
    for i=1:size(nList,2)
        y(i) = TaylorFunction(f, dfdt, t0, tn, y0, nList(i));
        err(i) = abs(fexact(tn) - y(i));
    end
    disp(y);
end

function x = TaylorFunction(f, dfdt, t0, tn, y0, n)
    h=(tn-t0)/n;
    while t0<=tn
        y1 = y0 + h*f(t0,y0) + (h^2/2)*dfdt(t0,y0);
        t1=t0+h;
        t0=t1;
        y0=y1;  
    end
